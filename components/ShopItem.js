...
export default function ShopItem({title, description, ImageSrc, PaymentType, selected}) {
    return (
        <>
            <div className={`p-5 ${selected ? 'border-blue-500/50' : 'border-white/50'} shadow shadow-black backdrop-blur-sm border-2 rounded-xl font-clash max-w-96`}>
            ...
            <div className={`text-center`}>
                <p className={`${selected ? 'inline-block' : 'hidden'} font-clash font-medium text-white p-2 border-r-2 border-b-2 border-l-2 backdrop-blur-sm border-blue-500/50 rounded-b-lg`}>Plus populaire</p>
            </div>
        </>
    )
}